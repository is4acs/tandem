...
export default function GuestbookMessage({ entry }: GuestbookMessageProps) {
  return (
    <article className="bg-white rounded-xl p-5 shadow-sm border border-mountain/10">
      ...
      <div className="flex items-center justify-between text-sm">
        ...
        <time className="text-bistro/40" dateTime={entry.created_at}>{formatDate(entry.created_at)}</time>
      </div>
    </article>
  );
}
